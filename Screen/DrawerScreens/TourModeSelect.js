...
const TourModeSelect = ({navigation}) => {
  ...
  const getTouristDestinations = city => {
    ...
    axios
      .get(
        `https://maps.googleapis.com/maps/api/place/textsearch/json?query=tourist+attractions+in+${city}&key=${GOOGLE_MAPS_API_KEY}`,
      )
      .then(response => {
        ...
        const minLat = Math.min(
          ...topLocations.map(location => location.latitude),
        );
        const maxLat = Math.max(
          ...topLocations.map(location => location.latitude),
        );
        const minLng = Math.min(
          ...topLocations.map(location => location.longitude),
        );
    ...
    <View style={styles.container}>
      ...
      {destinations.length > 0 && (
        <View style={styles.destinationsContainer}>
          <Text style={styles.destinationsHeading}>
            Top 5 Tourist Destinations to {city}
          </Text>
      ...
      {mapRegion ? (
        <View
          style={{flex: 5, borderRadius: 40, overflow: 'hidden', zIndex: -1}}>
        ...
        <View
          ...
          <ActivityIndicator
            ...
            style={{zIndex: -1}}
          />
          <Text style={{zIndex: -1}}>Loading Your City's Top Spots</Text>
        </View>
      )}
      {city && (
